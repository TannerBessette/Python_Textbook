---
title: "Olympics Project in Python"
...
```{python}
(
    ggplot(Male_prop, aes(x='Sport_Category', 
                                  y='country_code', 
...
Try slicing the Female prop data before plotting:
```{python}
Female_prop.head()
# .iloc accesses specific rows, can use it to basicall slice data:
sliced_Female_prop = Female_prop.iloc[0:285]
```

Figured out big issue: If number of levels on y axis and x axis are not 
equal, the squares are still proportional squares, so the heatmap just 
becomes a rectangle.

Maybe ggsize() argument will work to help fix as well?


Female Proportions Heatmap:
```{python}
(
    ggplot(sliced_Female_prop, aes(x='Sport_Category', 
                                  y='country_code', 
                                  fill='prop_sport_medals')) +  
    geom_tile() +
    ggsize(800, 1600) +
...
Skeleton for making a heatmap from ChatGPT (to help find errors):
```{python}
# Set seed for reproducibility
#np.random.seed(42)

# Generate 40 sports and 40 countries
#sports = [f"Sport_{i}" for i in range(1, 501)]
#countries = [f"Country_{i}" for i in range(1, 501)]

# Create a DataFrame with all combinations of sports and countries
#data = pd.DataFrame([(sport, country, np.random.randint(1, 600))
#                     for sport in sports for country in countries],
#                    columns=['Sport_Category', 'country_code', 'count'])

#(
#    ggplot(data, aes(x='Sport_Category', y='country_code', fill='count')) +
#    geom_tile() +
#    labs(title="Simple Heatmap of Medal Counts",
#         x="Sport",
#         y="Country") +
#    theme_minimal() +
#    scale_fill_gradient(low="lightblue", high="darkblue", name="Medal Count") +
#    theme(axis_text_x=element_text(angle=90, vjust=0.5, hjust=1, size=6))
#)
```
